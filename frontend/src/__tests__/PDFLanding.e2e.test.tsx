...
describe("PDFLanding end-to-end quality", () => {
  ...
  it("shows error if form is submitted empty", () => {
    ...
    expect(screen.getByText((content, node) => {
      return node && node.textContent && node.textContent.includes("Por favor completá todos los campos");
    })).toBeInTheDocument();
  });
  ...
  it("shows error for invalid email", async () => {
    ...
    expect(await screen.findByText((content, node) => {
      return node && node.textContent && node.textContent.includes("Por favor ingresá un email válido");
    })).toBeInTheDocument();
  });
